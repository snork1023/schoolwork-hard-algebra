...
const parsePrimaryAccent = (value: string) => {
  const match = value.trim().match(/(\d+)\s+(\d+)%\s+(\d+)%/);
  if (!match) {
    return { hue: 263, saturation: 70, lightness: 50 };
  }
...
const getAccentConfig = () => {
  if (typeof window === "undefined") {
    return { hue: 263, saturation: 0.7, lightness: 0.5 };
  }

  const computed = getComputedStyle(document.documentElement).getPropertyValue("--primary").trim();
  const { hue, saturation, lightness } = parsePrimaryAccent(computed || "263 70% 50%");
  return {
    hue,
    saturation,
    lightness,
...
void main(){
    ...
    if (uThemeMode < 0.5) {
        col.rgb=mix(vec3(1.0), col.rgb, 0.2);
    }
...
export default function DarkVeil({
  ...
  noiseIntensity: noiseIntensityProp,
  scanlineIntensity: scanlineIntensityProp,
  speed = 0.5,
  scanlineFrequency: scanlineFrequencyProp,
  warpAmount: warpAmountProp,
  resolutionScale = 1,
}: DarkVeilProps) {
  ...
  useEffect(() => {
    const update = () => {
      setAccentConfig(getAccentConfig());
  ...
  const hueShift = hueShiftProp ?? accentConfig.hue;
  const noiseIntensity = noiseIntensityProp ?? (isDark ? 0.01 + accentConfig.saturation * 0.03 : 0.005 + accentConfig.saturation * 0.02);
  const scanlineIntensity = scanlineIntensityProp ?? (isDark ? 0.03 + (1 - accentConfig.lightness) * 0.08 : 0.012 + (1 - accentConfig.lightness) * 0.04);
  const scanlineFrequency = scanlineFrequencyProp ?? (isDark ? 8 + accentConfig.saturation * 12 : 4 + accentConfig.saturation * 8);
  const warpAmount = warpAmountProp ?? (isDark ? 0.08 + accentConfig.saturation * 0.1 : 0.04 + accentConfig.saturation * 0.06);
