                  ...
                  <div className="space-y-1 max-w-4xl mx-auto">
                    {messages.map((message, index) => {
                      ...
                      const timeDiffToNext = nextMessage 
                        ? (new Date(nextMessage.created_at).getTime() - new Date(message.created_at).getTime()) / 1000 / 60 
                        : Infinity;
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-start gap-2">
                            <div
                              className={`inline-block px-4 py-2 max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg ${
                                message.user_id === user?.id
                                  ? "bg-primary text-primary-foreground"
                                  : "bg-muted"
                              } ${
                                isGroupedWithPrev && isGroupedWithNext
                                  ? message.user_id === user?.id ? "rounded-l-lg rounded-r-md" : "rounded-r-lg rounded-l-md"
                                  : isGroupedWithPrev
                                    ? message.user_id === user?.id ? "rounded-l-lg rounded-tr-md rounded-br-lg" : "rounded-r-lg rounded-tl-md rounded-bl-lg"
                                    : isGroupedWithNext
                                      ? message.user_id === user?.id ? "rounded-l-lg rounded-tr-lg rounded-br-md" : "rounded-r-lg rounded-tl-lg rounded-bl-md"
                                      : "rounded-lg"
                              }`}
                            >
